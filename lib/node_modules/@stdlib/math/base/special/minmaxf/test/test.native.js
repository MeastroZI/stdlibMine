...
tape( 'the function returns `NaN` for both the minimum and maximum value if provided a `NaN`', opts, function test( t ) {
	...
	v = minmaxf( NaN, 3.14 );
	t.strictEqual( isnanf( v[ 0 ] ), true, 'returns NaN' );
	t.strictEqual( isnanf( v[ 1 ] ), true, 'returns NaN' );
...
tape( 'the function returns correctly signed zeros', opts, function test( t ) {
	...
	v = minmaxf( +0.0, -0.0 );
	t.strictEqual( isNegativeZerof( float64ToFloat32( v[ 0 ] ) ), true, 'returns -0' );
	t.strictEqual( isPositiveZerof( float64ToFloat32( v[ 1 ] ) ), true, 'returns +0' );
	...
	v = minmaxf( -0.0, -0.0 );
	t.strictEqual( isNegativeZerof( float64ToFloat32( v[ 0 ] ) ), true, 'returns -0' );
	t.strictEqual( isNegativeZerof( float64ToFloat32( v[ 1 ] ) ), true, 'returns -0' );

	v = minmaxf( +0.0, +0.0 );
	t.strictEqual( isPositiveZerof( float64ToFloat32( v[ 0 ] ) ), true, 'returns +0' );
	t.strictEqual( isPositiveZerof( float64ToFloat32( v[ 1 ] ) ), true, 'returns +0' );
