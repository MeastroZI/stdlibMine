...
#### stdlib_base_dists_cauchy_cdf( p, x0, gamma )

Evaluates the quantile function for a Cauchy distribution with location parameter.
...
int main( void ) {
    ...
    for ( i = 0; i < 25; i++ ) {
        ...
        printf( "x: %lf, k: %lf, γ: %lf, Q(x;x0,γ): %lf\n", p, x0, gamma, y );
    }
}
```
