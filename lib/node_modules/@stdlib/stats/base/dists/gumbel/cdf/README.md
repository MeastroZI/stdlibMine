...
#### stdlib_base_dists_gumbel_cdf( x, sigma )

Evaluates the cumulative distribution function (CDF) for a Gumbel distribution 

```c
double out = stdlib_base_dists_gumbel_cdf( 2.0, 3.0 );
// returns ~0.199
```
...
```c
double stdlib_base_dists_gumbel_cdf( const double x, const double sigma );
```
